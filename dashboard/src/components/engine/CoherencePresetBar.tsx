/*
 * CoherencePresetBar.tsx — save/apply/export/import the full Coherence Engine tunable set.
 *
 * Mirrors PresetBar.tsx, but presets are app-side (no BLE write): "Apply" pushes the
 * preset's tunables into the store/engine, "Save as…" captures the current tunable set.
 */
...
import {
  ...
  listUserPresets,
  saveUserPreset,
  type SavedCoherencePreset,
} from './coherencePresetStore';
...
export default function CoherencePresetBar() {
  ...
  const [userPresets, setUserPresets] = useState<SavedCoherencePreset[]>([]);
  const [selectedId, setSelectedId] = useState<string>(BUILT_IN_PRESETS[0].id);
  ...
  const refreshUserPresets = useCallback(async () => {
    try {
      setUserPresets(await listUserPresets());
    } catch (err) {
  ...
  const selected: SavedCoherencePreset | undefined =
    BUILT_IN_PRESETS.find((p) => p.id === selectedId) ?? userPresets.find((p) => p.id === selectedId);

  const onApply = () => {
    if (!selected) return;
    setCoherenceTunables(selected.tunables);
    if (selected.mode && selected.mode !== engineMode) void setEngineMode(selected.mode);
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      await refreshUserPresets();
      setSelectedId(BUILT_IN_PRESETS[0].id);
  ...
  const onExport = () => {
    ...
    try {
      ...
      a.download = `${safe}.coherence.json`;
  ...
  const onImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const saved = await importPresetJson(await file.text());
      await refreshUserPresets();
      setSelectedId(saved.id);
  ...
  return (
    <div className="rounded border border-slate-800 bg-slate-900/50 p-3 flex flex-col gap-2 text-[11px]">
      <div className="text-[12px] font-medium text-slate-200">Engine presets</div>
      <select
        value={selectedId}
      ...
      >
        ...
        {userPresets.length > 0 ? (
          <optgroup label="Saved">
            {userPresets.map((p) => (
              <option key={p.id} value={p.id}>{p.name}{p.mode ? ` · ${p.mode}` : ''}</option>
            ))}
          </optgroup>
        ) : null}
      </select>
      <div className="flex flex-wrap gap-1.5">
        <button type="button" onClick={onApply} disabled={!selected || busy}
          className="rounded bg-emerald-600 hover:bg-emerald-500 px-2 py-1 text-[11px] font-medium disabled:opacity-40">
          Apply
        </button>
        <button type="button" onClick={() => { setSaveName(''); setShowSaveModal(true); }} disabled={busy}
          className="rounded bg-slate-700 hover:bg-slate-600 px-2 py-1 text-[11px] disabled:opacity-40">
          Save as…
        </button>
        <button type="button" onClick={() => void onDelete()} disabled={!selected || selected.builtIn || busy}
          className="rounded bg-slate-800 hover:bg-rose-700 px-2 py-1 text-[11px] disabled:opacity-40">
          Delete
        </button>
        <button type="button" onClick={onExport} disabled={!selected || busy}
      ...
      {status.kind !== 'idle' ? (
        <div className={`text-[10px] ${status.kind === 'err' ? 'text-rose-400' : status.kind === 'ok' ? 'text-emerald-400' : 'text-slate-300'}`}>
          {status.msg}
        </div>
      ) : null}

      {showSaveModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70" onClick={() => setShowSaveModal(false)}>
          <div className="w-80 rounded border border-slate-700 bg-slate-900 p-4 flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
            <div className="text-[12px] font-medium text-slate-100">Save engine preset</div>
            <input autoFocus type="text" value={saveName}
              onChange={(e) => setSaveName(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter') void onSave(); else if (e.key === 'Escape') setShowSaveModal(false); }}
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button type="button" onClick={() => setShowSaveModal(false)}
                className="rounded bg-slate-800 hover:bg-slate-700 px-2 py-1 text-[11px]">Cancel</button>
              <button type="button" onClick={() => void onSave()} disabled={!saveName.trim() || busy}
                className="rounded bg-emerald-600 hover:bg-emerald-500 px-2 py-1 text-[11px] font-medium disabled:opacity-40">Save</button>
            </div>
          </div>
        </div>
      ) : null}

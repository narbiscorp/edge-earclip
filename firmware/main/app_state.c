...
static uint8_t          s_pre_ota_state = APP_STATE_STREAMING;
...
{
    uint8_t p, f;
    esp_err_t err = config_get_last_mode(&p, &f);
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        const narbis_runtime_config_t *cfg = config_get();
        return app_state_request_mode(cfg->ble_profile, cfg->data_format);
    }
    if (err != ESP_OK) return err;
    return app_state_request_mode(p, f);
}
...
esp_err_t app_state_request_mode(uint8_t ble_profile, uint8_t data_format)
{
    ...
    transition(APP_STATE_STREAMING);
